...
public class OrderCancelFailedNotificationConsumer {

    private final SendNotificationUseCase sendNotificationUseCase;
    private final ObjectMapper objectMapper;

    @KafkaListener(
            topics = KafkaTopicConstants.ORDER_CANCEL_FAILED,
            groupId = "notification-order"
    )
    public void handleOrderCancelFailedEvent(
        ...
        if (EventPayloadValidator.hasEventTypeMismatch(envelope, KafkaTopicConstants.ORDER_CANCEL_FAILED)) {
            acknowledgment.acknowledge();
            return;
        }

        OrderCancelFailedEvent payload = envelope.getPayloadAs(OrderCancelFailedEvent.class, objectMapper);

        log.info("주문 취소 실패 이벤트 수신 (알림 발송). eventId={}, orderId={}, buyerId={}",
        ...
        log.info("주문 취소 실패 알림 발송 완료. orderId={}, buyerId={}", payload.orderId(), payload.buyerId());
