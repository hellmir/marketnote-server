...
public class OneOnOneInquiryAnsweredNotificationConsumer {

    private static final String ONE_ON_ONE_INQUERY_BOARD = "ONE_ON_ONE_INQUERY";
    ...
    @KafkaListener(
            ...
            groupId = "notification-one-on-one-inquiry-answered"
    )
    public void handleInquiryAnsweredEvent(
        ...
        if (!ONE_ON_ONE_INQUERY_BOARD.equals(payload.board())) {
            acknowledgment.acknowledge();
            return;
        }

        log.info("1:1 문의 답변 이벤트 수신 (알림 발송). eventId={}, userId={}, postId={}",
        ...
        try {
            ...
            log.info("1:1 문의 답변 알림 발송 완료. userId={}, postId={}", payload.userId(), payload.postId());
        } catch (Exception e) {
            log.error("1:1 문의 답변 알림 발송 실패. eventId={}, userId={}, postId={}, error={}",
