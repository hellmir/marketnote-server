...
class PurchaseConfirmationCompletedInAppNotificationConsumerTest {

    @InjectMocks
    private PurchaseConfirmationCompletedInAppNotificationConsumer consumer;
    ...
    private ConsumerRecord<String, EventEnvelope<?>> buildRecord(
            Long orderId, Long buyerId, boolean isAutoConfirmed) {
        OrderPurchaseConfirmedEvent event = new OrderPurchaseConfirmedEvent(
                orderId, buyerId, List.of(UUID.randomUUID()), isAutoConfirmed
        );
    ...
    @Test
    @DisplayName("자동 구매 확정 이벤트 수신 시 자동 확정 인앱 알림을 발송하고 acknowledge한다")
    void shouldSendAutoConfirmationInAppNotificationAndAcknowledge() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(100L, 1L, true);
        ...
        assertThat(command.templateCode()).isEqualTo("PURCHASE_CONFIRMATION_COMPLETED_AUTO");
        assertThat(command.deliveryChannel()).isEqualTo("IN_APP_ONLY");
    ...
    @Test
    @DisplayName("수동 구매 확정 이벤트 수신 시 수동 확정 인앱 알림을 발송하고 acknowledge한다")
    void shouldSendManualConfirmationInAppNotificationAndAcknowledge() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(200L, 2L, false);
        ...
        assertThat(command.templateCode()).isEqualTo("PURCHASE_CONFIRMATION_COMPLETED_MANUAL");
        assertThat(command.deliveryChannel()).isEqualTo("IN_APP_ONLY");
        assertThat(command.variables()).containsEntry("order_id", "200");
        assertThat(command.scheduledAt()).isNull();
    ...
    void shouldSkipWhenBuyerIdIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(100L, null, true);
    ...
    void shouldSkipWhenOrderIdIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(null, 1L, false);
