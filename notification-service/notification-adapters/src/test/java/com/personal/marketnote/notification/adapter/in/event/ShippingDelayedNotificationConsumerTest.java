...
class ShippingDelayedNotificationConsumerTest {

    @InjectMocks
    private ShippingDelayedNotificationConsumer consumer;
    ...
    @Test
    @DisplayName("배송 상태 DELIVERY_FAILED 이벤트 수신 시 SendNotificationUseCase를 호출하고 acknowledge한다")
    void shouldSendNotificationAndAcknowledgeWhenDeliveryFailedStatus() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(100L, 1L, "DELIVERY_FAILED");
        ...
        assertThat(command.templateCode()).isEqualTo("SHIPPING_DELAYED");
    ...
    void shouldSkipWhenEventTypeMismatch() {
        // given
        ShippingStatusChangedEvent event = new ShippingStatusChangedEvent(
                100L, 1L, "DELIVERY_FAILED", "TRACK-001", "CJ",
                LocalDateTime.of(2026, 4, 10, 15, 0)
        );
        EventEnvelope<ShippingStatusChangedEvent> envelope = new EventEnvelope<>(
    ...
    void shouldSkipWhenBuyerIdIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(100L, null, "DELIVERY_FAILED");
    ...
    void shouldSkipWhenOrderIdIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(null, 1L, "DELIVERY_FAILED");
