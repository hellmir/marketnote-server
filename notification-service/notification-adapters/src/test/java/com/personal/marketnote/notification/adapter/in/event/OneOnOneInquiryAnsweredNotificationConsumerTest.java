...
class OneOnOneInquiryAnsweredNotificationConsumerTest {

    @InjectMocks
    private OneOnOneInquiryAnsweredNotificationConsumer consumer;
    ...
    @Test
    @DisplayName("1:1 문의 답변 이벤트 수신 시 문의 작성자에게 푸시 알림을 발송하고 acknowledge한다")
    void shouldSendNotificationToInquiryAuthor() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(10L, 200L, "배송 문의", "ONE_ON_ONE_INQUERY");
    ...
    @Test
    @DisplayName("board가 PRODUCT_INQUERY이면 알림을 발송하지 않고 acknowledge한다")
    void shouldSkipWhenBoardIsProductInquery() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(10L, 200L, "상품 문의", "PRODUCT_INQUERY");
    ...
    void shouldSkipWhenEventTypeMismatch() {
        // given
        InquiryAnsweredEvent event = new InquiryAnsweredEvent(10L, 200L, "문의", "ONE_ON_ONE_INQUERY");
        EventEnvelope<InquiryAnsweredEvent> envelope = new EventEnvelope<>(
                "test-event-id", "wrong.event.type", "community-service",
                LocalDateTime.of(2026, 4, 10, 10, 0), event
        );
        ConsumerRecord<String, EventEnvelope<?>> record = new ConsumerRecord<>(
                "community.inquiry.answered", 0, 0L, "key-1", envelope
        );
    ...
    void shouldSkipWhenUserIdIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(null, 200L, "문의", "ONE_ON_ONE_INQUERY");
    ...
    void shouldSkipWhenPostIdIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(10L, null, "문의", "ONE_ON_ONE_INQUERY");
    ...
    void shouldSkipWhenTitleIsNull() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(10L, 200L, null, "ONE_ON_ONE_INQUERY");
    ...
    void shouldAcknowledgeEvenWhenSendFails() {
        // given
        ConsumerRecord<String, EventEnvelope<?>> record = buildRecord(10L, 200L, "문의", "ONE_ON_ONE_INQUERY");
