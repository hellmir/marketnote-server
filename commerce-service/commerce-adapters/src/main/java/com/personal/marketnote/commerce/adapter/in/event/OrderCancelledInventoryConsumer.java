...
public class OrderCancelledInventoryConsumer {
    ...
    @KafkaListener(
            ...
            groupId = "commerce-order-cancelled-inventory"
    )
    public void handleOrderCancelledEvent(
        ...
        try {
            ...
            log.info("주문 취소 이벤트 수신 (재고 복구). eventId={}, orderId={}, isFullCancel={}",
            ...
            log.error("주문 취소 재고 복구 이벤트 처리 실패. eventId={}, key={}, error={}",
    ...
    private void handleFullCancelInventoryRestore(EventEnvelope<?> envelope, OrderCancelledEvent payload) {
        ...
        restoreProductInventoryUseCase.restore(orderProducts, payload.orderId(), "주문 취소 전액 재고 복구");

        log.info("주문 취소 전체 재고 복구 완료. orderId={}, orderProducts={}건",
                payload.orderId(), payload.orderProducts().size());
    }

    private void handlePartialCancelInventoryRestore(EventEnvelope<?> envelope, OrderCancelledEvent payload) {
        ...
        restoreProductInventoryUseCase.restore(cancelOrderProducts, payload.orderId(), "주문 취소 부분 재고 복구");

        log.info("주문 취소 부분 재고 복구 완료. orderId={}, cancelProducts={}건",
